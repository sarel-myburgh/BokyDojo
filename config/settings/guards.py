...
def assert_safe_production_config(
    *,
    ...
    # ⚠ This used to refuse to boot when MFA enforcement was off. It no longer
    # does, because enrolment is now optional by decision (see base.py): an
    # organisation with no smartphone to hand could otherwise not sign in at
    # all, and with no SMTP there is no reset mail either. Privileged accounts
    # without a second factor are nudged on every page instead of locked out.
    #
    # ⚠ This is a real reduction in the guarantee SEC 2.4 asked for. What
    # replaces it is encouragement, not enforcement; an organisation wanting the
    # old behaviour sets DJANGO_MFA_ENFORCEMENT=true.
